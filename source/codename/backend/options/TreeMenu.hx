...
class TreeMenu extends MusicBeatState {
	...
	public var pathLabel:FlxText;
	public var pathDesc:FlxText;
	public var pathBG:FlxSprite;

	public static var lastState:Class<MusicBeatState> = null;  // Static for fixing the softlock bugs when resetting the state  - Nex
	...
	public override function create() {
		...
		pathLabel = new FlxText(4, 4, FlxG.width - 8, "> Tree Menu", 32, true);
		pathLabel.borderSize = 1.25;
		pathLabel.scrollFactor.set();
		pathLabel.font = Paths.font("gaposiss.ttf");

		pathDesc = new FlxText(4, pathLabel.y + pathLabel.height + 2, FlxG.width - 8, "Current Tree Menu Description", 16, true);
		pathDesc.scrollFactor.set();
		pathDesc.font = Paths.font("gaposiss.ttf");
	...
	public function exit() {
		FlxG.switchState((lastState != null) ? Type.createInstance(lastState, []) : new states.modded.DaveMain());
		lastState = null;
	}
	...
	public override function update(elapsed:Float) {
		...
		pathLabel.x = FlxMath.lerp(pathLabel.x, Math.max(0, FlxG.width - 4 - pathLabel.width), 0.125);
	}
